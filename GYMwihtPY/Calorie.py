...
def open_breakfastcalories():
     root.destroy()
     os.system('python breakfastCalories.py')  # Assuming main() is the entry point in breakfastCalories.py

def open_thirdwindow():
    root.destroy()
    os.system('python third_window.py')  # Assuming third_window.py is standalone

def open_secondwindow():
    root.destroy()
    subprocess.Popen(['python', 'third_window.py'])  # Run second_window.py as a separate process

def open_firstwindow():
    root.destroy()
    subprocess.Popen(['python', 'first_window.py'])  # Run first_window.py as a separate process
...
icon = Image.open(r"Assets\torso.png")
