...
def dumbels():
    ...
    try:
        cursor.execute("INSERT INTO workouts (visitor_id,workout) VALUES (?,?)", (id_entry.get(), "Back dumbels only"))
        conn.commit()
...
def widelats():
    ...
    try:
        cursor.execute("INSERT INTO workouts (visitor_id,workout) VALUES (?,?)", (id_entry.get(),"back wide lats"))
        conn.commit()
    except Exception as e:
        print(e)
...
def go_back():
    root.destroy()
    subprocess.run(['python', 'workout.py'])
