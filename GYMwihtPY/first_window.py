...
def insert_data(result,id_entry):
    ...
    ssn=id_entry.get()
    cursor = conn.cursor()
    cursor.execute("UPDATE calories set lunch_cal=?WHERE visitor_id=?", (result, ssn))
    conn.commit()
...
# Main function for the first window
def main_first():
    food_items = [
    ...
    id_label.place(x=375, y=450)
    id_entry = tk.Entry(root, font=("Arial", 15), fg="#000", bg="#fff", width=10, highlightcolor='gray',highlightthickness=5, highlightbackground='gray')
    id_entry.place(x=370, y=480)

    total_label = tk.Label(root, text="Total: 0 cal", fg="black", font=("Arial", 14))
    total_label.place(x=200, y=600)
    
    total_button = tk.Button(root, text="Total", command=lambda: calculate_total(entries, food_items, total_label,id_entry), bg="gray", fg="black", font=("Arial", 12))
    total_button.place(x=200, y=550)

    reset_button = tk.Button(root, text="Reset", command=lambda: reset_entries(entries, total_label,id_entry), bg="gray", fg="black", font=("Arial", 12))
    reset_button.place(x=200, y=650)

    # Create the backward arrow button to go to system.py
    left_button =tk.Button(root, text="<<<", command=lambda: open_system_window(root), bg="black", fg="white", font=("Arial", 20, "bold"), bd=0)
    left_button.place(x=10, y=640)
    root.mainloop()
