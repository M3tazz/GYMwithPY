...
def reset_entries(entries, total_label,id_entry):
    ...
    insert_data(0,id_entry)

# Function to switch to the system.py
def open_system_window(current_root):
    current_root.destroy()
    os.system('python system.py')  # Ensure system.py is in the same directory or provide the correct path
...
def third_window():
    ...
    total_button = tk.Button(root, text="Total", command=lambda: calculate_total(entries, food_items, total_label,id_entry), bg="gray", fg="black", font=("Comic Sans MS", 12))
    total_button.place(x=200, y=550)

    reset_button = tk.Button(root, text="Reset", command=lambda: reset_entries(entries, total_label,id_entry), bg="gray", fg="black", font=("Comic Sans MS", 12))
